...
function LogoCirclePreview() {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-dashed p-3">
      <Label htmlFor="logo-preview-input">Vista previa de logo circular (no sube nada)</Label>
      <input
...
function ContrastWarning({ brandColorHex }: { brandColorHex: string }) {
  ...
  if (check.passesForeground && check.passesLabel) {
    return <p className="text-xs text-success">Contraste suficiente contra el texto fijo del pase de Apple.</p>;
  }
  return (
...
export function BrandingPanel({ businessId, branding }: { businessId: string; branding: Branding }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      ...
      {state.success ? (
        <Alert>
          <AlertDescription>Branding actualizado.</AlertDescription>
        </Alert>
      ) : null}
    </form>
  );
