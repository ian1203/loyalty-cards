...
// reales que ya tienen un pase de Google guardado) al classId _v6 — la
// clase nueva con merchantLocations activado, con las 3 coordenadas
// reales confirmadas (CHILAQUIKES local, foodtruck Torrente, foodtruck
// Calasanz — ver locations.latitude/longitude en la DB real). Regla no
...
// y esta migración NO borra la clase _v5, solo deja de ser la que los
// clientes ven.
//
...
// migrate-google-class-v5.ts, este script NO consulta la base de datos.
...
//
// Uso: pnpm --filter @loyalty/wallet exec node --env-file-if-exists=../../apps/web/.env.local --experimental-strip-types scripts/migrate-google-class-v6.ts
...
if (!config.google.credentials) {
  console.error(
    `[migrate-v6] Google Wallet no está activo. Faltan: ${config.google.status.missing.join(", ")}.\n` +
...
// businesses (slug='chilaquikes') — mismos valores que migrate-google-class-v5.ts.
...
// Coordenadas reales confirmadas — locations.latitude/longitude en la DB
// real (business_id=BUSINESS_ID), 2026-08-11.
const MERCHANT_LOCATIONS = [
  { latitude: 19.175369, longitude: -96.1212448 }, // Local (Av. Cristóbal Colón)
  { latitude: 19.1552634, longitude: -96.1311204 }, // Foodtruck Torrente
  { latitude: 19.1242781, longitude: -96.14227 }, // Foodtruck Calasanz
];
...
// (2026-08-11) — 7 clientes con fila de wallet_passes(platform='google'),
// Alessandra Rosas Carmona se agregó desde la migración _v5.
...
async function main() {
  ...
  console.log(`[migrate-v6] Creando la clase nueva: ${classPayload.id}`);
  await client.upsertLoyaltyClass(classPayload.id as string, classPayload);
  console.log(`[migrate-v6] Clase _v6 lista (con merchantLocations, ${MERCHANT_LOCATIONS.length} ubicaciones).`);

  const oldClassId = buildLoyaltyClassId(issuerId, BUSINESS_ID, "v5");
  console.log(`[migrate-v6] Clase vieja (${oldClassId}) queda intacta — no se borra.`);
  ...
  for (const customer of CUSTOMERS) {
    ...
    console.log(`[migrate-v6] Migrando ${customer.fullName} (${objectId}) → ${objectPayload.classId}`);
    await client.upsertLoyaltyObject(objectId, objectPayload);
  }

  console.log(`\n[migrate-v6] Listo — ${CUSTOMERS.length} clientes reales migrados a ${classPayload.id}.`);
}

main().catch((error) => {
  console.error("[migrate-v6] Falló:", error);
  process.exit(1);
});
