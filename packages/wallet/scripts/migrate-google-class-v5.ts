...
// reales que ya tienen un pase de Google guardado) al classId _v5 — la
// clase nueva con el nombre REAL del programa, "Club de la Gorrita"
// (restaurado en loyalty_programs.name; el genérico "Tarjeta de sellos
// Chilaquikes" que traían _v2/_v3/_v4 nunca fue el nombre real, nadie lo
// había pedido así — ver loyaltyPayload.ts). classTemplateInfo (accountName
// en la cara, _v4) se conserva sin cambios. Regla no negociable de la
// tarea: clase NUEVA con classId NUEVO, nunca un PATCH in-place de la
// clase vieja (Google cachea agresivamente por classId) — y esta
// migración NO borra la clase _v4, solo deja de ser la que los clientes ven.
//
...
// migrate-google-class-v4.ts, este script NO consulta la base de datos.
...
//
// Uso: pnpm --filter @loyalty/wallet exec node --env-file-if-exists=../../apps/web/.env.local --experimental-strip-types scripts/migrate-google-class-v5.ts
...
if (!config.google.credentials) {
  console.error(
    `[migrate-v5] Google Wallet no está activo. Faltan: ${config.google.status.missing.join(", ")}.\n` +
...
// businesses (slug='chilaquikes') — mismos valores que migrate-google-class-v4.ts.
...
// Nombre REAL del programa — confirmado con el dueño, ver loyalty_programs.name.
...
// (2026-08-11). Si vuelves a correr este script más tarde, refresca de
...
// reward_rules activas — mismo criterio que v4 (replica availableRewards
...
async function main() {
  ...
  console.log(`[migrate-v5] Creando la clase nueva: ${classPayload.id}`);
  await client.upsertLoyaltyClass(classPayload.id as string, classPayload);
  console.log(`[migrate-v5] Clase _v5 lista (programName="${PROGRAM_NAME}").`);

  const oldClassId = buildLoyaltyClassId(issuerId, BUSINESS_ID, "v4");
  console.log(`[migrate-v5] Clase vieja (${oldClassId}) queda intacta — no se borra.`);
  ...
  for (const customer of CUSTOMERS) {
    ...
    console.log(`[migrate-v5] Migrando ${customer.fullName} (${objectId}) → ${objectPayload.classId}`);
    await client.upsertLoyaltyObject(objectId, objectPayload);
  }

  console.log(`\n[migrate-v5] Listo — ${CUSTOMERS.length} clientes reales migrados a ${classPayload.id}.`);
}

main().catch((error) => {
  console.error("[migrate-v5] Falló:", error);
  process.exit(1);
});
